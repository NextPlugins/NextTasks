...
public final class NextTasksAPI {
    ...
    @Getter
    private static final NextTasksAPI instance = new NextTasksAPI();
    ...
    public Optional<Task> findTaskByFilter(Predicate<Task> filter) {
        return allTasks().stream().filter(filter).findFirst();
    }
    ...
    public Set<Task> findTasksByFilter(Predicate<Task> filter) {
        return allTasks().stream().filter(filter).collect(Collectors.toSet());
    }
